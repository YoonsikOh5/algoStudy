		...
		whileloop : while(q.size()>0) {
			...
			for(int d = 0; d < 4; d++) {
				...
				if(nr>=0 && nr<n && nc>=0 && nc<m) {
					...
					} else if(map[nr][nc]=='0' && cur.bcnt==0 && visited[nr][nc]==2) {
						visited[nr][nc] = 1;
						step[nr][nc] = step[curr][curc]+1;
						if(nr == n-1 && nc == m-1) {
							min_route = Math.min(min_route, step[nr][nc]);
					...
					if(map[nr][nc]=='1' && visited[nr][nc]==0 && cur.bcnt==0 && (wallls[nr][nc]==false)) {
						visited[nr][nc] = 2;
						wallls[nr][nc]=true;
						step[nr][nc] = step[curr][curc]+1;
