...
public class Main {
	...
	public static void perm(int[][] board, int depth, int curmax) {
		// 기저조건
		if(curmax <= (result_max/(Math.pow(2,(10-depth))))){
			return;
		}
		if(depth == 10 || result_max==maxcand) {
			return;
		}
		int nextcurmax = curmax;
		// 유도파트
		for(int i = 1; i <= 4; i++) {
			int[][] nboard = move(board,i, nextcurmax);
			nextcurmax = stcurmax;
			perm(nboard,depth+1, nextcurmax);
			nextcurmax = curmax;
		}
	...
	public static int[][] move(int[][] board, int direction, int curmax) {
		stcurmax = curmax; 
					...
					if (curnum == -1) {
						...
						if (temp[r][c] != 0) {
							if (temp[r][c] == curnum) {
								...
								stcurmax = Math.max(stcurmax, mergenum);
								temp[r][c] = 0;
								curnum = -1;
								curnumidx = -1;
							} else {
